...
import com.micropole.homemodule.R.id.tv_search_style
...
class SearchActivity : BaseRecyclerActivity<SearchBean.ProjectBean, SearchStyleConstract.Model, SearchStyleConstract.View, SearchStyleConstract.Present>(), SearchStyleConstract.View {
    ...
    override fun getStyleData(data: List<SearchStyleBean>?) {
        if (data != null && data.isNotEmpty()) {
            val searchStyleBean = SearchStyleBean()
            searchStyleBean.area_id = "0"
            searchStyleBean.area_name = "所有风格"
            mData.add(searchStyleBean)
            mData.addAll(data)
            tv_search_style.text = mData[0].area_name
            mstyleId = mData[0].area_id
            onRefresh()
        }
    }
